...
std::map<std::wstring, bool> grayList;
    ...
    *size = 1024;
    ...
    if (!whiteList[program_name]) {
        if (!blackList[program_name]) {
            if (!grayList[program_name]) {
                grayList[program_name] = true;
...
bool EnumDirs(std::wstring node) {
    ...
    HANDLE hFind = FindFirstFileW((node + L"\\*").c_str(), &rootData);
    ...
    do {
        ...
        if (rootData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            if (next_dir.compare(L".") != 0 && next_dir.compare(L"..") != 0) {
                std::wstring programDir(node + L"\\" + next_dir);
                EnumDirs(programDir);
...
bool EnumDirs(std::wstring node) {
    ...
    do {
        std::wstring next_dir(rootData.cFileName);
        std::wstring programPath(node + L"\\" + next_dir);

        if (rootData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            if (next_dir.compare(L".") != 0 && next_dir.compare(L"..") != 0) {
                EnumDirs(programPath);
            } 
        }
        else {
            LPDWORD lpBinaryType = new DWORD;
...
int wmain() {
    ...
    roots.push_back(mainDrive + L"\\Windows");
